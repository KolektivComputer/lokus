package cat.mey.platform.links.services

import cat.mey.platform.core.auth.data.entities.Account
import cat.mey.platform.links.data.entities.Link
...
class LinkService : KoinComponent {
    ...
    suspend fun generateShortIdSuspend(
    ...
    fun linkExistsByVanitySuspend(account: Account, vanity: String) = suspendTransaction {
        Link.findByVanity(account, vanity) != null
    }
    ...
    fun createLink(account: Account, destination: Url) = transaction {
        val shortId = generateShortId(
            exists = {
                shortId -> linkExistsByShortId(account, shortId)
            }
        )
