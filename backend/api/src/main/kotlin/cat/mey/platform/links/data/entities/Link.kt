...
import cat.mey.platform.links.data.tables.LinksTable
...
class Link(id: EntityID<Uuid>) : UuidEntity(id) {
    ...
    companion object : UuidEntityClass<Link>(LinksTable) {
        fun findByVanityOrShortId(account: Account, searchTerm: String) = find {
            (LinksTable.account eq account.id) and ((LinksTable.vanity eq searchTerm) or (LinksTable.shortId eq searchTerm))
        }.firstOrNull()
